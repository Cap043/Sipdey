...
- The site is now based on Next.js. Is a much better fit than Create React App. For now I am just using it to create a static export, but maybe I'll add some server rendered stuff in the future.
- Styling is now vanilla CSS with postcss to add support for the future native CSS nesting and custom media queries features. I am using CSS modules instead of BEM syntax to avoid style conflicts.
- For generating pages from `.mdx` files, I am using Kent C Dodds' [mdx-bundler](https://github.com/kentcdodds/mdx-bundler). In combination with Next.js it makes generating pages from `.mdx` files really quick and simple.
